...
it('validates required fields when creating a budget', function () {
    ...
    $response = $this->actingAs($user)->from(route('budgets.create'))->post(route('budgets.store'), [
        'name' => '',
        'amount' => '',
        'type' => ''
    ]);
...
it('create a budget and redirect with success message', function () {
    $user = User::factory()->create([
        'email_verified_at' => now()
    ]);
...
it('accept valid budget type', function () {
    $user = User::factory()->create([
        'email_verified_at' => now()
    ]);
